...
class TestDefinitionSelector:
    ...
    def test_by_key_as_str_by_language(self, db_session):
        definitions = (
            DefinitionSelector().by_key("test").by_language("es").all(db_session)
        )
    ...
    def test_disable_model_check_false(self, db_session):
        with pytest.raises(ValueError) as _:
            DefinitionSelector(model=BaseWord, disable_model_check=False).all(
                db_session
            )
    ...
    def test_disable_model_check_false_with_subclass(self, db_session):
        ...
        result = DefinitionSelector(
            model=TestDefinition, disable_model_check=False
        ).scalar(db_session)
        assert isinstance(result, TestDefinition)

    def test_by_key_raise_error(self, db_session):
        with pytest.raises(AttributeError) as _:
            DefinitionSelector(model=BaseWord, disable_model_check=True).by_key(
                "act"
            ).scalar(db_session)

    def test_by_language_raise_error(self, db_session):
        with pytest.raises(AttributeError) as _:
            DefinitionSelector(model=BaseWord, disable_model_check=True).by_language(
                "en"
            ).scalar(db_session)
