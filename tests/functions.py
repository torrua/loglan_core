...
from loglan_core import Word, Key, Event, Author, Type, Definition
...
def db_add_object(session: Session, obj_class, obj):
    obj = obj_class(**obj)
    session.add(obj)
    session.commit()


def db_add_objects(session: Session, obj_class, objects):
    class_objects = [obj_class(**obj) for obj in objects]
    session.bulk_save_objects(class_objects)
    session.commit()
...
def db_connect_words(session: Session, pairs: list):
